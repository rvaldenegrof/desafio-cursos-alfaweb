...
export const cursos = {
  ...
  getters: {
    cantidadTotalAlumnosPermitidos(state) {
      return state.data.reduce((acc, curso) => {
        acc += Number.parseInt(curso.cupos)
        return acc
      }, 0)
    },
    cantidadTotalAlumnosInscritos(state) {
      return state.data.reduce((acc, curso) => {
        acc += Number.parseInt(curso.inscritos)
        return acc
      }, 0)
    },
    cantidadTotalCuposRestantes(state) {
      return state.data.reduce((acc, curso) => {
        acc += Number.parseInt(curso.cupos) - Number.parseInt(curso.inscritos)
        return acc
      }, 0)
    },
